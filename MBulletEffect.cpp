...
MBulletEffect::MBulletEffect(Texture texture, Sound* sound, D3DXVECTOR3 pos)
{
	Start(texture, sound, pos);
}

MBulletEffect::~MBulletEffect()
{
}

void MBulletEffect::Start(Texture texture, Sound* sound, D3DXVECTOR3 pos)
{
	m_bActive = true;
...
void MBulletEffect::Update(float deltaTime)
{
	if (m_bActive)
	{
		Animation(2, 4, 0.05f, deltaTime);
	}
}

void MBulletEffect::Animation(int rowNum, int lastNum, float delayTime, float deltaTime)
{
